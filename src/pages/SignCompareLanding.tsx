...
import {
  signCompareVariantBySlug,
  signCompareVariants,
  type SignCompareVariantConfig,
  type SignCompareFeatureRow,
} from "@/data/signCompareVariants";
...
function ComparisonTable({ title, rows, competitor }: { title: string; rows: SignCompareFeatureRow[]; competitor: string }) {
  return (
...
function buildJsonLd(variant: SignCompareVariantConfig) {
...
const SignCompareLanding = () => {
  const { pathname } = useLocation();
  const slug = pathname.replace(/^\/+/, "");
  const variant = signCompareVariantBySlug[slug];
        ...
        <div className="max-w-5xl mx-auto px-6 py-20 md:py-28 space-y-20 md:space-y-28">
          <motion.section {...fadeUp} className="text-center max-w-3xl mx-auto">
            ...
            <div className="flex items-center justify-center gap-3">
              <Link to="/sign" className={cn("inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-primary-foreground bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all duration-200 active:scale-[0.98]")}>
                <Upload className="w-4 h-4" />
                Try Docsora Sign
              </Link>
              <Link to="/sign" className="text-xs font-medium text-muted-foreground/80 hover:text-primary transition-colors">Back to Sign hub</Link>
            </div>
          </motion.section>
          ...
          <ComparisonTable title="Workflow speed" rows={variant.workflow} competitor={variant.competitor} />
          <ComparisonTable title="Security & compliance" rows={variant.security} competitor={variant.competitor} />
          <ComparisonTable title="Experience" rows={variant.experience} competitor={variant.competitor} />
          <ComparisonTable title="Pricing model" rows={variant.pricing} competitor={variant.competitor} />
          ...
          <motion.section {...fadeUp}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-4xl mx-auto">
              {signCompareVariants.filter((v) => v.slug !== variant.slug).map((v) => (
          ...
          <motion.section {...fadeUp}>
            <div className="text-center rounded-3xl p-12 md:p-16 bg-card/40 border border-border/30 backdrop-blur-sm">
              ...
              <p className="text-sm text-muted-foreground/70 mb-8 max-w-md mx-auto">Drop a document into Docsora Sign and run a real signing workflow. No installs, no plugins.</p>
              <Link to="/sign" className={cn("inline-flex items-center justify-center gap-2 px-7 py-3 rounded-xl text-sm font-semibold text-primary-foreground bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all duration-200 active:scale-[0.98]")}>
                <Upload className="w-4 h-4" />
                Try Docsora Sign
...
export default SignCompareLanding;
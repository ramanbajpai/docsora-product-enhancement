...
import {
  convertCompareVariantBySlug,
  convertCompareVariants,
} from "@/data/convertCompareVariants";
import type { CompareVariantConfig, CompareFeatureRow } from "@/data/compareVariants";
...
function buildJsonLd(variant: CompareVariantConfig) {
...
const ConvertCompareLanding = () => {
  const { pathname } = useLocation();
  const slug = pathname.replace(/^\/+/, "");
  const variant = convertCompareVariantBySlug[slug];
    ...
    <AppLayout>
      ...
      <div className="relative bg-background min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-6 py-20 md:py-28 space-y-20 md:space-y-28">
          ...
          <ComparisonTable title="Conversion quality" rows={variant.quality} competitor={variant.competitor} />
          ...
          <motion.section {...fadeUp}>
            <div className="text-center rounded-3xl p-12 md:p-16 bg-card/40 border border-border/30 backdrop-blur-sm">
              ...
              <p className="text-sm text-muted-foreground/70 mb-8 max-w-md mx-auto">
                Drop a file into Docsora and compare the experience side-by-side. No signup, no installs.
              </p>
              <Link
                to="/convert"
                className={cn(
...
export default ConvertCompareLanding;
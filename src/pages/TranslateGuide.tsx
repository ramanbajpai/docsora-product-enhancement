...
import { ArrowRight, ChevronRight, Languages, BookOpen, Clock3 } from "lucide-react";
...
import { translateGuideBySlug } from "@/data/translateGuides";
...
const TranslateGuide = () => {
  ...
  const guide = translateGuideBySlug[slug];
  ...
  const canonical = `/translate-guides/${guide.slug}`;
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Guides", item: "/translate" },
          ...
          <motion.section {...fadeUp} className="mt-24">
            ...
            >
              ...
              <p className="text-sm text-muted-foreground/75 mb-7 max-w-md mx-auto">
                Try {guide.primaryToolLabel} — browser-based, secure, and finished in seconds.
              </p>
              <Link
                to={`/${guide.primaryToolSlug}`}
                className={cn(

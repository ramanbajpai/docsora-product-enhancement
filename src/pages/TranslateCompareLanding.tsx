...
import { ArrowRight, Check, Minus, Sparkles, Zap, Languages } from "lucide-react";
...
import {
  translateCompareVariantBySlug,
  translateCompareVariants,
  type TranslateCompareVariantConfig,
  type TranslateCompareFeatureRow,
} from "@/data/translateCompareVariants";
...
function ComparisonTable({
  ...
  title: string;
  rows: TranslateCompareFeatureRow[];
  competitor: string;
}) {
  return (
...
function buildJsonLd(variant: TranslateCompareVariantConfig) {
...
const TranslateCompareLanding = () => {
  const { pathname } = useLocation();
  const slug = pathname.replace(/^\/+/, "");
  const variant = translateCompareVariantBySlug[slug];
    ...
    <AppLayout>
      ...
      <div className="relative bg-background min-h-screen">
        ...
        <div className="max-w-5xl mx-auto px-6 py-20 md:py-28 space-y-20 md:space-y-28">
          ...
          <ComparisonTable title="Supported formats" rows={variant.formats} competitor={variant.competitor} />
          <ComparisonTable title="Security & compliance" rows={variant.security} competitor={variant.competitor} />
          <ComparisonTable title="Workflow experience" rows={variant.experience} competitor={variant.competitor} />
          ...
          <motion.section {...fadeUp}>
            <div className="text-center rounded-3xl p-12 md:p-16 bg-card/40 border border-border/30 backdrop-blur-sm">
              ...
              <p className="text-sm text-muted-foreground/70 mb-8 max-w-md mx-auto">
                Drop a document into Docsora and translate it into 75+ languages without losing formatting.
              </p>
              <Link
                to="/translate"
                className={cn(
...
export default TranslateCompareLanding;
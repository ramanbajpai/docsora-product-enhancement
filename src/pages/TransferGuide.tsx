...
import { ArrowRight, ChevronRight, Send, BookOpen, Clock3 } from "lucide-react";
...
import { transferGuideBySlug } from "@/data/transferGuides";
...
const TransferGuide = () => {
  ...
  const guide = transferGuideBySlug[slug];
  ...
  const canonical = `/transfer-guides/${guide.slug}`;
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Guides", item: "/transfer" },
    ...
    <AppLayout>
      ...
      <article className="bg-background">
        <div className="max-w-3xl mx-auto px-6 pt-16 pb-24 md:pt-24 md:pb-32">
          ...
          >
            <Link to="/transfer" className="hover:text-primary transition-colors">
              Transfer
            </Link>
              ...
              <motion.section {...fadeUp} key={section.heading}>
                ...
                <div className="space-y-4">
                  {section.paragraphs.map((p, i) => (
                    <p key={i} className="text-[15px] text-muted-foreground/85 leading-[1.75]">
                      {p}
                    </p>
                  ))}
                  {section.bullets && (

...
interface RecipientTransferViewProps {
  onClose: () => void;
}

// Mock transfer data - expanded for better demo
const mockTransfer = {
  sender: {
    name: "Alex Chen",
...
export function RecipientTransferView({ onClose }: RecipientTransferViewProps) {
  ...
  // Smooth spring animation for cursor glow - more inertia, softer feel
  ...
  // Card transforms for parallax (very subtle)
  ...
  // Ambient background gradient animation phase
  const [gradientPhase, setGradientPhase] = useState(0);
  
  useEffect(() => {
    const interval = setInterval(() => {
      setGradientPhase(prev => (prev + 1) % 360);
    }, 100); // ~25 second full cycle
  ...
  const handleDownload = () => {
    ...
    // Collapse animation then prepare
    setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setStatusText("Downloading securely…");
    }, 1400);
  };
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText("https://docsora.app/transfer/abc123");
  };

  const isDownloading = downloadState !== "idle" && downloadState !== "complete";
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-background via-background to-background overflow-hidden">
        ...
        {/* Ambient gradients with slow drift animation */}
        <motion.div 
      ...
      {/* Cursor-following glow - reduced intensity with organic noise texture */}
      <motion.div
      ...
      />
      {/* Secondary subtle glow layer for organic feel */}
      <motion.div
      ...
      {/* Main Content - Centered Hero Card */}
      <main className="relative z-10 flex items-center justify-center min-h-[calc(100vh-180px)] px-6">
        ...
        >
          {/* Hero Card - Enhanced glassmorphism with refined shadows */}
          <motion.div 
          ...
          >
            ...
            <div className="relative bg-card/85 backdrop-blur-lg rounded-3xl shadow-[0_20px_60px_-20px_rgba(0,0,0,0.4),inset_0_1px_0_0_rgba(255,255,255,0.03),inset_0_-1px_0_0_rgba(0,0,0,0.1)]">
              {/* Refined inner glow - top highlight */}
              ...
              <div className="relative p-8">
                {/* Sender Identity - Enhanced hierarchy */}
                ...
                {/* Sender Message - calmer, more intentional */}
                ...
                <AnimatePresence mode="wait">
                  {(downloadState === "idle" || downloadState === "complete") && (
                    ...
                    >
                      ...
                      >
                        {/* Grouped file count and size as a unit */}
                ...
                <AnimatePresence mode="wait">
                  ...
                  {downloadState === "complete" && (
                    ...
                    >
                      {/* Success Animation - compact, restrained */}
                      <motion.div
          ...
          {/* Security badge below card - only show when not complete */}
          {downloadState !== "complete" && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}

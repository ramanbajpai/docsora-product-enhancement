...
function ClientWorkspacePreview({
  ...
  const signer = roles.find((r) => (r.type ?? "signer") === "signer");
  const signerName = signer?.name || "Client";
  const subject = applyTemplateVariables(delivery.emailSubject ?? "Please sign {{TEMPLATE_NAME}}", sample);
  const askVars = variables.filter((v) => v.required).length ? variables.filter((v) => v.required) : variables;

  const steps = [
    { key: "email", label: "Email", icon: Mail },
    { key: "welcome", label: "Welcome", icon: Sparkles },
    { key: "fill", label: "Your details", icon: Type },
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[80] bg-background/80 backdrop-blur-xl flex items-center justify-center p-4"
        ...
        >
          <motion.div
            ...
            transition={{ duration: 0.25 }}
            onClick={(e) => e.stopPropagation()}
            className="w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-3xl border border-border/60 bg-card shadow-2xl flex flex-col"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-border/50">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-[12.5px] font-semibold">Client workspace preview</div>
                  <div className="text-[10.5px] text-muted-foreground">What {signerName} will experience</div>
                </div>
              </div>
              <button
                onClick={() => onOpenChange(false)}
                className="text-[11.5px] text-muted-foreground hover:text-foreground px-2 py-1 rounded-md hover:bg-muted/60"
              >
                Close
              </button>
            </div>
                  ...
                  return (
                    <div key={s.key} className="flex items-center gap-1.5 flex-1">
                      <button
                        onClick={() => setStep(i)}
                        className={cn(
                          "flex items-center gap-1.5 px-2 py-1 rounded-md text-[10.5px] font-medium transition-colors",
                      ...
                      {i < steps.length - 1 && (
                        <div className={cn("h-px flex-1", done ? "bg-primary/40" : "bg-border/60")} />
                      )}
                    </div>
                  );
                })}
                ...
                >
                  {current.key === "email" && (
                    <div className="max-w-xl mx-auto space-y-3">
                      <div className="text-[10.5px] uppercase tracking-wider text-muted-foreground font-semibold">Inbox</div>
                      <div className="rounded-2xl border border-border/50 bg-background p-5 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="text-[12px] text-muted-foreground">From: you@company.com</div>
                          <div className="text-[11px] text-muted-foreground">now</div>
                        </div>
                        <div className="text-[16px] font-semibold">{subject || `Please sign ${templateName}`}</div>
                        <p className="text-[13px] text-muted-foreground leading-relaxed">
                          Hi {signerName}, please review and sign the document below. It only takes a minute.
                        </p>
                        <button
                          onClick={next}
                          className="inline-flex items-center gap-2 h-9 px-4 rounded-lg bg-primary text-primary-foreground text-[12.5px] font-medium hover:opacity-90"
                        >
                  ...
                  {current.key === "welcome" && (
                    <div className="max-w-xl mx-auto text-center space-y-4 py-6">
                      ...
                      <h3 className="text-[24px] font-semibold tracking-tight">Welcome, {signerName}</h3>
                      <p className="text-[13.5px] text-muted-foreground">
                        You&rsquo;ve been invited to review and sign <span className="font-medium text-foreground">{templateName}</span>.
                      </p>
                      <div className="flex items-center justify-center gap-3 pt-1">
                        <div className="text-[11.5px] text-muted-foreground inline-flex items-center gap-1.5">
                          <FileStack className="w-3.5 h-3.5" /> {documents.length} document{documents.length === 1 ? "" : "s"}
                        </div>
                        <div className="text-[11.5px] text-muted-foreground inline-flex items-center gap-1.5">
                  ...
                  {current.key === "fill" && (
                    <div className="max-w-xl mx-auto space-y-4">
                      <div>
                        <h3 className="text-[18px] font-semibold tracking-tight">A few quick details</h3>
                        <p className="text-[12.5px] text-muted-foreground">We&rsquo;ll use these to personalize your document.</p>
                      </div>
                      {askVars.length === 0 ? (
                        <div className="rounded-xl border border-dashed border-border/60 p-5 text-[12.5px] text-muted-foreground text-center">
                          Nothing to fill in — document is ready.
                        </div>
                      ) : (
                        askVars.slice(0, 6).map((v) => (
                          <div key={v.name} className="space-y-1.5">
                            <label className="text-[12.5px] font-medium">{launchQuestionFor(v)}</label>
                            <div className="h-10 rounded-lg border border-border/60 bg-background flex items-center px-3 text-[12.5px] text-muted-foreground">
                              {v.defaultValue || `Enter ${v.label.toLowerCase()}`}
                            </div>
                          </div>
                        ))
                  ...
                  {current.key === "review" && (
                    <div className="max-w-2xl mx-auto space-y-3">
                      <div>
                        <h3 className="text-[18px] font-semibold tracking-tight">Review the document</h3>
                        <p className="text-[12.5px] text-muted-foreground">Everything looks correct? Continue to sign.</p>
                      </div>
                      <div className="rounded-2xl border border-border/50 bg-background p-6 space-y-3 max-h-[300px] overflow-y-auto">
                        <div className="text-[15px] font-semibold">{templateName}</div>
                        {(DOC_BODY_BY_TAG[documents[0]?.tag ?? "agreement"] ?? DOC_BODY_BY_TAG.agreement).paragraphs.slice(0, 3).map((p, i) => (
                          <p key={i} className="text-[12.5px] text-muted-foreground leading-relaxed">{p}</p>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  {current.key === "sign" && (
                    <div className="max-w-xl mx-auto space-y-4">
                      <div>
                        <h3 className="text-[18px] font-semibold tracking-tight">Add your signature</h3>
                        <p className="text-[12.5px] text-muted-foreground">Draw or type your signature below.</p>
                      </div>
                      <div className="h-32 rounded-2xl border-2 border-dashed border-border/60 bg-background flex items-center justify-center">
                        <div className="text-center space-y-1">
                          <PenLine className="w-5 h-5 text-muted-foreground mx-auto" />
                          <div className="text-[12px] text-muted-foreground">Sign here</div>
                        </div>
                      </div>
                      <div className="text-[11px] text-muted-foreground inline-flex items-center gap-1.5">
                        <ShieldCheck className="w-3 h-3" /> Legally binding & timestamped
                      </div>
                    </div>
                  )}
                  ...
                  {current.key === "done" && (
                    <div className="max-w-xl mx-auto text-center space-y-4 py-8">
                      ...
                      <h3 className="text-[22px] font-semibold tracking-tight">You&rsquo;re all done</h3>
                      <p className="text-[13px] text-muted-foreground">
                        A signed copy has been emailed to everyone involved.
                      </p>
                    </div>
                  )}
                </motion.div>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-border/50 bg-muted/20">
              <button
                onClick={prev}
                disabled={step === 0}
                className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-medium text-muted-foreground hover:text-foreground disabled:opacity-40 disabled:pointer-events-none"
              >
                <ArrowLeft className="w-3.5 h-3.5" /> Back
              </button>
              ...
              {step < steps.length - 1 ? (
                <button
                  onClick={next}
                  className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg bg-primary text-primary-foreground text-[12px] font-medium hover:opacity-90"
                >
                  Continue <ArrowRight className="w-3.5 h-3.5" />
                </button>
              ) : (

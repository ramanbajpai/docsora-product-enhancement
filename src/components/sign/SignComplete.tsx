...
const continueActions = [
  ...
  { icon: FileArchive, label: "Compress", path: "/compress" },
...
const toolsSubmenu = [
  { icon: RotateCw, label: "Rotate", path: "/tools/rotate" },
  { icon: SplitSquareHorizontal, label: "Split", path: "/tools/split" },
  { icon: FileText, label: "Merge", path: "/tools/merge" },
  { icon: Trash2, label: "Delete", path: "/tools/delete-pages" },
...
const SignComplete = ({ file, onReset }: SignCompleteProps) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto py-10 px-4">
      ...
      >
        ...
        >
          <span className="text-foreground/80">Legally binding</span>
          <span className="mx-2.5 text-muted-foreground/30">•</span>
          <span className="text-foreground/80">Timestamped</span>
          <span className="mx-2.5 text-muted-foreground/30">•</span>
          <span className="text-foreground/80">Tamper-proof</span>
        </motion.p>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <span className="relative z-10">Download Signed Document</span>
        </motion.button>
        ...
        <div className="relative rounded-2xl overflow-hidden">
          ...
          <div className="relative px-6 py-5 backdrop-blur-xl">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-foreground whitespace-nowrap">
                    Unlock Pro — advanced signing features
                  </h3>
                  <p className="text-xs text-muted-foreground/70 mt-0.5 whitespace-nowrap">
                    Bulk signing • Custom branding • Audit trails
                  </p>
                </div>
              </div>
              ...
              >
                <motion.div
                  className="absolute inset-0 bg-gradient-to-r from-transparent via-primary/8 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-500"
                />
      ...
      >
        <p className="text-[10px] text-muted-foreground/40">
          Your document is encrypted and securely stored. Compliant with ESIGN Act & eIDAS regulations.
        </p>
      </motion.div>
    </div>
  );
